...
export class PdfService {
  ...
  private tableHeader(doc: PDFKit.PDFDocument, headers: string[], widths: number[], y: number) {
  ...
  private tableRow(doc: PDFKit.PDFDocument, cells: string[], widths: number[], y: number, isOdd: boolean) {
    if (isOdd) doc.rect(40, y, 515, 16).fill('#f8f9ff');
    let x = 40;
  ...
  async genererRapportStock(data: any, res: Response) {
    ...
    data.chambres.forEach((c: any, i: number) => {
      if (y > 750) { doc.addPage(); y = 50; y = this.tableHeader(doc, headers, widths, y); }
  ...
  async genererRapportClients(clients: any[], res: Response) {
    ...
    clients.forEach((c: any, i: number) => {
      if (y > 750) { doc.addPage(); y = 50; y = this.tableHeader(doc, headers, widths, y); }
      y = this.tableRow(doc,
        [c.clientNom, c.telephone || '', String(c.stockActuel), String(c.totalEntree), String(c.totalSortie)],
        widths, y, i % 2 === 0
      );
    });
  ...
  async genererRapportPaiements(data: any, res: Response) {
    ...
    if (data.paiements) {
      data.paiements.forEach((p: any, i: number) => {
        if (y > 750) { doc.addPage(); y = 50; y = this.tableHeader(doc, headers, widths, y); }
        y = this.tableRow(doc,
          [p.datePaiement, p.client?.nom || '', p.modePaiement, p.reference || '', `${Number(p.montant).toFixed(2)} €`],
          widths, y, i % 2 === 0
        );
      });
    }
