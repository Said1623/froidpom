...
export default function EntreePage() {
  ...
  // Calcul quota par client
  const clientsAvecReste = useMemo(() => {
    if (!clients || !reservations || !entrees) return [];
    return clients.map(client => {
      const resa = reservations.find(r => r.client.id === client.id && isInCampagne((r as any).dateReservation));
      if (!resa) return null;
      const resaB = resa.nbCaissesBois || 0;
      const resaP = (resa as any).nbCaissesPластique || 0;
      const resaT = (resa as any).nbCaissesTranger || 0;
      const dejaB = entrees.filter(e => e.client.id === client.id && e.typeCaisse === 'bois' && isInCampagne(e.dateEntree)).reduce((s: number, e: any) => s + e.nbCaisses, 0);
      const dejaP = entrees.filter(e => e.client.id === client.id && e.typeCaisse === 'plastique' && isInCampagne(e.dateEntree)).reduce((s: number, e: any) => s + e.nbCaisses, 0);
      const dejaT = entrees.filter(e => e.client.id === client.id && e.typeCaisse === 'tranger' && isInCampagne(e.dateEntree)).reduce((s: number, e: any) => s + e.nbCaisses, 0);
      const resteB = Math.max(0, resaB - dejaB);
      const resteP = Math.max(0, resaP - dejaP);
      const resteT = Math.max(0, resaT - dejaT);
      const totalReste = resteB + resteP + resteT;
      return { client, resaB, resaP, resaT, resteB, resteP, resteT, totalReste };
    }).filter(Boolean).filter(c => c!.totalReste > 0) as {
      client: Client; resaB: number; resaP: number; resaT: number;
      resteB: number; resteP: number; resteT: number; totalReste: number;
    }[];
  }, [clients, reservations, entrees]);

  const clientData = useMemo(() => {
    if (!clientId) return null;
    return clientsAvecReste.find(c => String(c.client.id) === clientId) || null;
  }, [clientId, clientsAvecReste]);
    ...
    <div className="fade-in" style={{ maxWidth: 620, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border)', borderRadius: 16, padding: '20px 24px', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--c-text3)', marginTop: 2 }}>
                Campagne {campagneActive} — {clientsAvecReste.length} client(s) à affecter
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 4, background: 'var(--c-bg2)', borderRadius: 10, padding: 4 }}>
            ...
            <button onClick={() => setTab('historique')} style={{ padding: '7px 16px', borderRadius: 7, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', background: tab === 'historique' ? 'var(--c-primary)' : 'transparent', color: tab === 'historique' ? '#fff' : 'var(--c-text2)' }}>📋 Historique ({(entrees || []).filter((e: any) => isInCampagne(e.dateEntree)).length})</button>
          </div>
        </div>
      </div>
      ...
      {tab === 'entree' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* 1. Client — seulement ceux avec restant > 0 */}
          <div style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border)', borderRadius: 14, padding: '18px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--c-text3)' }}>{clientsAvecReste.length} avec quota restant</div>
            </div>
            <select value={clientId} onChange={e => { setClientId(e.target.value); setQuantite(''); }}
              ...
              {clientsAvecReste.map(({ client, totalReste }) => (
                <option key={client.id} value={client.id}>{client.nom} — restant: {totalReste}</option>
              ))}
            </select>

            {/* Quota détaillé + Restant à affecter */}
            {clientData && (
              <>
                {/* Bandeau restant total */}
                <div style={{ marginTop: 12, padding: '10px 14px', background: 'rgba(46,207,138,.08)', border: '1px solid rgba(46,207,138,.25)', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 13, color: 'var(--c-text2)', fontWeight: 600 }}>📊 Restant à affecter</span>
                  <strong style={{ fontSize: 18, color: 'var(--c-success)' }}>{clientData.totalReste} caisses</strong>
                </div>
                {/* Détail par type */}
                <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {clientData.resaP > 0 && (
                    <div style={{ background: 'rgba(79,142,247,.08)', border: `1px solid rgba(79,142,247,.2)`, borderRadius: 8, padding: '8px 12px', flex: 1, minWidth: 90 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 800, color: '#4f8ef7' }}>{clientData.resteP}</div>
                      <div style={{ fontSize: 10, color: 'var(--c-text3)' }}>/ {clientData.resaP} réservés</div>
                    </div>
                  )}
                  {clientData.resaB > 0 && (
                    <div style={{ background: 'rgba(245,166,35,.08)', border: '1px solid rgba(245,166,35,.2)', borderRadius: 8, padding: '8px 12px', flex: 1, minWidth: 90 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 800, color: '#f5a623' }}>{clientData.resteB}</div>
                      <div style={{ fontSize: 10, color: 'var(--c-text3)' }}>/ {clientData.resaB} réservés</div>
                    </div>
                  )}
                  {clientData.resaT > 0 && (
                    <div style={{ background: 'rgba(0,212,180,.08)', border: '1px solid rgba(0,212,180,.2)', borderRadius: 8, padding: '8px 12px', flex: 1, minWidth: 90 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 800, color: '#00d4b4' }}>{clientData.resteT}</div>
                      <div style={{ fontSize: 10, color: 'var(--c-text3)' }}>/ {clientData.resaT} réservés</div>
                    </div>
                  )}
                </div>
              </>
          ...
          <div style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border)', borderRadius: 14, padding: '18px 20px' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {TYPES.map(t => {
                const reste = clientData ? t.value === 'bois' ? clientData.resteB : t.value === 'plastique' ? clientData.resteP : clientData.resteT : null;
                const dispo = reste === null || reste > 0;
                return (
                  <button key={t.value} onClick={() => { setType(t.value); setQuantite(''); }}
                    style={{ flex: 1, padding: '14px 8px', borderRadius: 12, cursor: dispo ? 'pointer' : 'not-allowed', opacity: !dispo ? 0.4 : 1, transition: 'all .15s', background: type === t.value ? t.bg : 'var(--c-bg2)', border: `2px solid ${type === t.value ? t.border : 'var(--c-border)'}`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 24 }}>{t.icon}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: type === t.value ? t.color : 'var(--c-text2)' }}>{t.label}</span>
                    {reste !== null && <span style={{ fontSize: 10, color: reste > 0 ? t.color : 'var(--c-danger)', fontWeight: 700 }}>{reste > 0 ? `${reste} restants` : 'Complet ✓'}</span>}
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ background: 'var(--c-surface)', border: `2px solid ${depasseQuota || depasseChambre ? 'var(--c-danger)' : 'var(--c-border)'}`, borderRadius: 14, padding: '18px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--c-text3)', textTransform: 'uppercase', letterSpacing: '.6px' }}>🔢 Quantité</div>
              {clientData && quotaReste > 0 && (
                <div style={{ fontSize: 12, color: typeActif.color, fontWeight: 700 }}>max: {quotaReste}</div>
              )}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      {tab === 'historique' && (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            {TYPES.map(t => {
              const n = filteredEntrees.filter((e: any) => e.typeCaisse === t.value).reduce((s: number, e: any) => s + e.nbCaisses, 0);
              if (!n) return null;
                  ...
                  <tr key={e.id} style={{ borderBottom: '1px solid var(--c-border)', background: i % 2 === 0 ? '' : 'rgba(255,255,255,.01)' }}>
                    ...
                    <td style={{ padding: '10px 8px' }}>
                      <button onClick={async () => {
                        if (!confirm(`Supprimer cette entrée (+${e.nbCaisses} ${e.typeCaisse} pour ${e.client.nom}) ?\n\nLe stock de la chambre sera mis à jour.`)) return;
                        try {
                          await entreesApi.delete(e.id);
                          toast.success('Entrée supprimée');
                          refetchEntrees(); refetchChambres();
                        } catch (err: any) {
                          toast.error(err?.response?.data?.message || 'Stock insuffisant en chambre — impossible de supprimer');
                        }
